...
class MutableCAB(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        super(MutableCAB, self).__init__()
        modules_body = []
        modules_body.append(conv(n_feat, n_feat, kernel_size, bias=bias))
        modules_body.append(act)
        modules_body.append(conv(n_feat, n_feat, kernel_size, bias=bias))
    ...
    def forward(self, x, windowSize):
        loopTime = windowSize // 2 - 2
        x_iter = x.clone()
        # res = x
        for _ in range(loopTime):
            res = self.body(x_iter)
            res = self.CA(res)
            res += x_iter
            x_iter = res
        # import torchvision
        # import ipdb; ipdb.set_trace()
        # torchvision.utils.save_image(res, "test01.png")
        return res
...
class Encoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, csff):
        ...
        self.encoder_level1_follow = MutableCAB(n_feat,                     kernel_size, reduction, bias=bias, act=act)
        self.encoder_level2_follow = MutableCAB(n_feat+scale_unetfeats,     kernel_size, reduction, bias=bias, act=act)
        self.encoder_level3_follow = MutableCAB(n_feat+(scale_unetfeats*2), kernel_size, reduction, bias=bias, act=act)
    ...
    def forward(self, x, window_size, encoder_outs=None, decoder_outs=None):
        enc1 = self.encoder_level1(x)
        enc1 = self.encoder_level1_follow(enc1, window_size)
        ...
        enc2 = self.encoder_level2_follow(enc2, window_size)
        if (encoder_outs is not None) and (decoder_outs is not None):
        ...
        enc3 = self.encoder_level3_follow(enc3, window_size)
        if (encoder_outs is not None) and (decoder_outs is not None):
...
class MPRNet(nn.Module):
    ...
    def forward(self, x3_img, window_sizes):
        ...
        feat1_lbot = self.stage1_encoder(x1lbot, window_size)
        feat1_rbot = self.stage1_encoder(x1rbot, window_size)
        ...
        feat2_top = self.stage2_encoder(x2top_cat, window_size, feat1_top, res1_top)
        feat2_bot = self.stage2_encoder(x2bot_cat, window_size, feat1_bot, res1_bot)
